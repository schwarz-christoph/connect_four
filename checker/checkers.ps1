...
cd $scriptdir
java --version
java --enable-preview -jar .\metrics.jar $sourceDir > .\report\metrics.log

cd $sourceDir
java -jar "$scriptdir\checkstyle-8.41.1-all.jar" -c "$scriptdir\checkstyle-2021-04-07.xml" -o $scriptdir\report\checkstyle.log .\

cd $scriptdir
.\pmd-bin-6.33.0\pmd-bin-6.33.0\bin\pmd.bat -d $sourceDir -f text -R .\pmd-2021-04-07.xml > .\report\pmd.log

$jacocoClassPath = "$testClasses;$classes;.\junit-platform-console-standalone-1.5.2.jar"
java -Xint -javaagent:.\jacoco-0.8.7\jacocoagent.jar=append=true,destfile=.\report\jacoco.exec -cp $jacocoClassPath org.junit.runner.JUnitCore $TEST_FQCN
java -jar .\jacoco-0.8.7\jacococli.jar report .\report\jacoco.exec --sourcefiles $sourceDir --classfiles $classes --html .\report\jacoco\
